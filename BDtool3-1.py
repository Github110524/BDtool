...
class BDToolsApp(tk.Tk):
    ...
    def create_default_download_dir(self):
        ...
        if sys.platform.startswith('win'):
            desktop = os.path.join(os.path.expanduser("~"), "D:/Desktop")
        else:  # macOS和Linux
    ...
    def create_ui(self):
        ...
        self.geometry("650x450")  # 增加高度以容纳详细进度信息
    ...
    def download_file(self, url, path, file_type=""):
        ...
        last_bytes = 0
        last_time = time.time()
        speed_history = []  # 存储最近5个速度样本
        update_interval = 0.5  # 每0.5秒更新一次速度显示
        ...
        try:
            ...
            def update_ui(progress, speed, eta, status):
                """更新UI的辅助函数"""
                self.progress.set(progress)
                self.status_label.config(text=status)
                self.detail_label.config(text=f"{progress:.1f}% - 速度: {speed} - 剩余时间: {eta}")
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if current_time - last_update >= update_interval:
                            ...
                            if elapsed > 0:
                                # 计算瞬时速度 (KB/s)
                                speed = (downloaded - last_bytes) / elapsed / 1024
                                ...
                                # 格式化下载速度显示
                                if avg_speed < 1024:
                                    speed_text = f"{avg_speed:.2f} KB/s"
                                else:
                                    speed_text = f"{avg_speed/1024:.2f} MB/s"
                                ...
                                # 使用after将UI更新推送到主线程
                                self.after(0, update_ui, progress_percent, speed_text, eta, status_text)
            ...
            self.after(0, update_ui, 100, "0.00 KB/s", self.format_time(total_time), self.get_text('download_complete'))
